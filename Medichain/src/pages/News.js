...
const News = () => {
  ...
  useEffect(() => {
    async function initializeNews() {
        const a = await axios.get('https://newsapi.org/v2/top-headlines?country=us&category=health&apiKey=014b35cf2f264a129261cb580be71916')
        console.log(a);
        setNews(a);
        setSpinner(false)
    }
            ...
            <Card maxW='sm' style={{height:"100%"}}>
              <CardBody>
                <Image
                  ...
                  src={`${news.data.articles[index].urlToImage ? news.data.articles[index].urlToImage : "/interaction.jpg"}`}
                  alt='Green double couch with wooden legs'
                  borderRadius='lg'
                />
                <Stack mt='6' spacing='3'>
                  <Heading size='md'>{news.data.articles[index].title}</Heading>
                  <Text>
                    {news.data.articles[index].description}
                  </Text>
